...
rt_callback(struct rtimer *t, void *ptr)
{
  rt_now = RTIMER_NOW();
  ct = clock_time();
  printf("Task called at %d (clock = %d)\n", rt_now, ct);
}
#endif
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(clock_test_process, ev, data)
{
  ...
  printf("clock_test_process entry...\n");
  ...
  while(i < 7) {
    ...
    printf("Requested: %d usec, Real: %ld rtimer ticks = ~%ld us\n",
        10000 * i, diff, DivU64x32 (diff, 2500));
    i++;
  }
#endif

#if TEST_RTIMER
  printf("Rtimer Test, 1 sec (%d rtimer ticks):\n", RTIMER_SECOND);
  i = 0;
  while(i < 5) {
    etimer_set(&et, 2 * CLOCK_SECOND);
    printf("=======================\n");
...
#if TEST_ETIMER
  printf("Clock tick and etimer test, 1 sec (%d clock ticks):\n", CLOCK_SECOND);
  i = 0;
  while(i < 10) {
