...
EDKII_SMM_CPU_PLATFORM_HOOK_PROTOCOL  *mSmmCpuPlatformHook = NULL;
...
{
  return mSmmCpuPlatformHook->PlatformValidSmi ();
}
...
{
  return mSmmCpuPlatformHook->ClearTopLevelSmiStatus ();
}
...
{
  return mSmmCpuPlatformHook->PlatformSmmBspElection (IsBsp);
}
...
{
  return mSmmCpuPlatformHook->GetPlatformPageTableAttribute (Address, PageSize, NumOfPages, PageAttribute);
}
...
EFI_STATUS
EFIAPI
SmmCpuPlatformHookLibConstructor (
