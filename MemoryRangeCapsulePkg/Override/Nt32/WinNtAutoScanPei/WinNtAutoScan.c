...
--*/
{
  ...
  if (FeaturePcdGet(PcdWinNtCapsuleEnable)) {
    if (Capsule != NULL) {
      ...
      //
      DEBUG ((EFI_D_ERROR, "Before Coalesce:\n"));
      ...
      DEBUG ((EFI_D_ERROR, "After Coalesce:\n"));
  ...
  DEBUG ((EFI_D_ERROR, "  PeiMemoryBase - %lx-%lx\n", PeiMemoryBase, PeiMemorySize));
  Hob.Raw = GetFirstGuidHob (&gEfiMemoryRangeCapsuleGuid);
  if (Hob.Raw != NULL) {
    Status = GetFreeMemoryFromMemoryRange ((VOID **)&PeiMemoryBase, (UINTN *)&PeiMemorySize, GET_GUID_HOB_DATA (Hob));
    ASSERT_EFI_ERROR (Status);
  }
  DEBUG ((EFI_D_ERROR, "  PeiMemoryBase(update) - %lx-%lx\n", PeiMemoryBase, PeiMemorySize));
