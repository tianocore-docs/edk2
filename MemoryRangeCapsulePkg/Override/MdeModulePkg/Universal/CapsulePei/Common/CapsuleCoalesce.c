...
VOID
DumpMemoryRangeCapsuleValid (
  IN EFI_MEMORY_RANGE_CAPSULE       *MemoryRangeCapsule
  )
{
  UINTN  Index;

  DEBUG ((DEBUG_INFO, "MemoryRangeCapsule:\n"));
  DEBUG ((DEBUG_INFO, "  OsRequestedMemoryType 0x%x\n", MemoryRangeCapsule->OsRequestedMemoryType));
  DEBUG ((DEBUG_INFO, "  MemoryForFirmwareUse  (0x%lx-0x%lx)\n", MemoryRangeCapsule->MemoryForFirmwareUse.Address, MemoryRangeCapsule->MemoryForFirmwareUse.Length));
  DEBUG ((DEBUG_INFO, "  NumberOfMemoryRanges  0x%lx\n", MemoryRangeCapsule->NumberOfMemoryRanges));
  for (Index = 0; Index < MemoryRangeCapsule->NumberOfMemoryRanges; Index++) {
    DEBUG ((DEBUG_INFO, "  MemoryRanges          (0x%lx-0x%lx)\n", MemoryRangeCapsule->MemoryRanges[Index].Address, MemoryRangeCapsule->MemoryRanges[Index].Length));
  }
}

BOOLEAN
IsMemoryRangeCapsuleValid (
  IN EFI_MEMORY_RANGE_CAPSULE       *MemoryRangeCapsule,
  IN UINTN                          MemoryRangeCapsuleSize
  )
{
  if (!CompareGuid (&MemoryRangeCapsule->Header.CapsuleGuid, &gEfiMemoryRangeCapsuleGuid)) {
    DEBUG ((DEBUG_ERROR, "IsMemoryRangeCapsuleValid: GUID - %g\n", &MemoryRangeCapsule->Header.CapsuleGuid));
    return FALSE;
  }
  if (MemoryRangeCapsule->Header.HeaderSize != sizeof(EFI_CAPSULE_HEADER)) {
    DEBUG ((DEBUG_ERROR, "IsMemoryRangeCapsuleValid: HeaderSize - 0x%x, expect - 0x%x\n", MemoryRangeCapsule->Header.HeaderSize, (UINTN)sizeof(EFI_CAPSULE_HEADER)));
    return FALSE;
  }
  if (MemoryRangeCapsule->Header.CapsuleImageSize != MemoryRangeCapsuleSize) {
    DEBUG ((DEBUG_ERROR, "IsMemoryRangeCapsuleValid: CapsuleImageSize - 0x%x, expect - 0x%x\n", MemoryRangeCapsule->Header.CapsuleImageSize, MemoryRangeCapsuleSize));
    return FALSE;
  }
  if (MemoryRangeCapsuleSize <= sizeof(EFI_MEMORY_RANGE_CAPSULE)) {
    DEBUG ((DEBUG_ERROR, "IsMemoryRangeCapsuleValid: MemoryRangeCapsuleSize too small - 0x%x, expect - 0x%x\n", MemoryRangeCapsuleSize, sizeof(EFI_MEMORY_RANGE_CAPSULE)));
    return FALSE;
  }
  if (MemoryRangeCapsule->NumberOfMemoryRanges != ((MemoryRangeCapsuleSize - sizeof(EFI_MEMORY_RANGE_CAPSULE)) / sizeof(EFI_MEMORY_RANGE))) {
    DEBUG ((DEBUG_ERROR, "IsMemoryRangeCapsuleValid: NumberOfMemoryRanges incorrect - 0x%lx, expect - 0x%x\n", MemoryRangeCapsule->NumberOfMemoryRanges, (MemoryRangeCapsuleSize - sizeof(EFI_MEMORY_RANGE_CAPSULE)) / sizeof(EFI_MEMORY_RANGE)));
    return FALSE;
  }
  return TRUE;
}
...
/**
  Get MemoryRangeCapsule.

  No sanity check is needed, because it is already done in GetCapsuleInfo and ValidateCapsuleIntegrity.

**/
EFI_MEMORY_RANGE_CAPSULE *
GetMemoryRangeCapsule (
  IN EFI_CAPSULE_BLOCK_DESCRIPTOR   *Desc,
  IN MEMORY_RESOURCE_DESCRIPTOR     *MemoryResource,
  IN UINTN                          NumDescriptors,
  IN UINTN                          CapsuleSize,
  IN UINTN                          CapsuleNumber,
  ...
  )
{
  ...
  BOOLEAN                        IsValid;
  VOID                           *CapsuleBuffer;
  UINTN                          CapsuleBufferSize;
  UINTN                          CapsuleBufferOffset;
  EFI_MEMORY_RANGE_CAPSULE       *MemoryRangeCapsule;

  DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: ScratchBuffer - 0x%x\n", ScratchBuffer));
  DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: ScratchBufferSize - 0x%x\n", ScratchBufferSize));
  if (CapsuleNumber > 1) {
    DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: CapsuleNumber > 1\n"));
    return NULL;
  }
  if (ScratchBufferSize < sizeof(EFI_MEMORY_RANGE_CAPSULE)) {
    DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: ScratchBufferSize too small\n"));
    return NULL;
  }

  //
  // Copy capsule data to ScratchBuffer
  //
  ...
  CapsuleBuffer = NULL;
  CapsuleBufferSize = 0;
  CapsuleBufferOffset = 0;
  MemoryRangeCapsule = NULL;
  ...
  while (Desc->Union.ContinuationPointer != (EFI_PHYSICAL_ADDRESS) (UINTN) NULL) {
    if (Desc->Length == 0) {
      ...
      //
      // This has been checked in GetCapsuleInfo()
      //
      ASSERT (Desc->Length < (MAX_ADDRESS - Size));
      Size += (UINTN) Desc->Length;
      Count++;
      ...
      if (ThisCapsuleImageSize == 0) {
        ...
        Number ++;
        ThisCapsuleImageSize = CapsuleHeader->CapsuleImageSize;
        ASSERT (ThisCapsuleImageSize == CapsuleSize);

        //
        // Check GUID
        //
        if (!CompareGuid (&CapsuleHeader->CapsuleGuid, &gEfiMemoryRangeCapsuleGuid)) {
          DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: CapsuleGuid mismatch - %g\n", &CapsuleHeader->CapsuleGuid));
          return NULL;
        }
        if (Desc->Length < sizeof(EFI_MEMORY_RANGE_CAPSULE)) {
          DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: Desc->Length too small - 0x%\n", Desc->Length));
          return NULL;
        }
        MemoryRangeCapsule = (VOID *)CapsuleHeader;
        DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: MemoryForFirmwareUse 0x%lx-0x%lx\n", MemoryRangeCapsule->MemoryForFirmwareUse.Address, MemoryRangeCapsule->MemoryForFirmwareUse.Length));
        if (MemoryRangeCapsule->MemoryForFirmwareUse.Address != 0 && MemoryRangeCapsule->MemoryForFirmwareUse.Length != 0) {
          if (!ValidateCapsuleByMemoryResource (MemoryResource, MemoryRangeCapsule->MemoryForFirmwareUse.Address, MemoryRangeCapsule->MemoryForFirmwareUse.Length)) {
            return NULL;
          }
          //
          // Save a copy to scratch buffer, so that MemoryForFirmwareUse can be found.
          //
          CopyMem (ScratchBuffer, (VOID *)(UINTN)Desc->Union.DataBlock, sizeof(EFI_MEMORY_RANGE_CAPSULE));

          CapsuleBuffer = (VOID *)(UINTN)MemoryRangeCapsule->MemoryForFirmwareUse.Address;
          CapsuleBufferSize = (UINTN)MemoryRangeCapsule->MemoryForFirmwareUse.Length;
        } else {
          CapsuleBuffer = ScratchBuffer;
          CapsuleBufferSize = ScratchBufferSize;
        }
        if (CapsuleBufferSize < CapsuleSize) {
          DEBUG ((DEBUG_INFO, "GetMemoryRangeCapsule: CapsuleBufferSize < CapsuleSize\n"));
          return NULL;
        }
      }
      ...
      CopyMem (
        (UINT8 *)((UINTN)CapsuleBuffer + CapsuleBufferOffset),
        (VOID *)(UINTN)Desc->Union.DataBlock,
        (UINTN)Desc->Length
        );
      CapsuleBufferOffset = (UINTN)(CapsuleBufferOffset + Desc->Length);
  ...
  //
  // This is guarantee in GetCapsuleInfo()
  //
  ASSERT (CapsuleSize == Size);
  ASSERT (CapsuleBufferOffset == Size);
  ...
  //
  // This is guarantee in the beginning.
  //
  ASSERT (Number == 1);

  //
  // Sanity check for MemoryRangeCapsule
  //
  IsValid = IsMemoryRangeCapsuleValid (CapsuleBuffer, CapsuleBufferOffset);
  if (!IsValid) {
    return NULL;
  }

  DEBUG_CODE_BEGIN();
  DumpMemoryRangeCapsuleValid (CapsuleBuffer);
  DEBUG_CODE_END();

  return CapsuleBuffer;
}
...
CapsuleImageBase-->+---------------------------+
  ...
  @param MemorySize         Size of the memory region pointed to by MemoryBase.
                            ...
                            coalesced capsule.
  @param ScratchBuffer      base of scratch memory
  @param ScratchBufferSize  size of scratch memory
  ...
  )
{
  ...
  DEBUG_CODE (
    CapsuleTestPatternPreCoalesce (BlockList);
  );
  ...
  //
  // Collect MemRangeInformation in capsule.
  //
  // We have to use scratch buffer to hold MemoryRangeCapsule, because we have not pass it yet.
  // We do not know which memory can be used.
  //
  ASSERT (ScratchBufferSize > sizeof(EFI_GUID));
  MemoryRangeCapsule = GetMemoryRangeCapsule (BlockList, MemoryResource, NumDescriptors, CapsuleSize, CapsuleNumber, ScratchBuffer, ScratchBufferSize);
  if (MemoryRangeCapsule != NULL) {
    if (MemoryRangeCapsule->MemoryForFirmwareUse.Address != 0 && MemoryRangeCapsule->MemoryForFirmwareUse.Length != 0) {
      ASSERT (MemoryRangeCapsule == (VOID *)(UINTN)MemoryRangeCapsule->MemoryForFirmwareUse.Address);
    } else {
      ASSERT (MemoryRangeCapsule == ScratchBuffer);
    }
    //
    // Update MemoryBase and MemorySize
    //
    Status = GetFreeMemoryFromMemoryRange (MemoryBase, MemorySize, MemoryRangeCapsule);
    return Status;
  } else {
    ZeroMem (ScratchBuffer, sizeof(EFI_GUID));
  }
  ...
  // processing easier.
  //
  BlockList = RelocateBlockDescriptors (BlockList, NumDescriptors, FreeMemBase, FreeMemSize);
  if (BlockList == NULL) {
    //
