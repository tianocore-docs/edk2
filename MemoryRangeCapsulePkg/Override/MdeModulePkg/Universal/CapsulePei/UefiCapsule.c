  ...
  )
{
  ...
  Context.ScratchBuffer         = (UINT64)(UINTN)ScratchBuffer;
  ...
  )
{
...
#endif
  UINTN                                TempBuffer[SIZEOF_SCRATCH_BUFFER/sizeof(UINTN)];
  VOID                                 *ScratchBuffer;
  ...
  ScratchBuffer = TempBuffer;
  ScratchBufferSize = sizeof(TempBuffer);
  ...
  if (FeaturePcdGet (PcdDxeIplSwitchToLongMode)) {
    ...
    Status = ModeSwitch (&LongModeBuffer, CoalesceEntry, (EFI_PHYSICAL_ADDRESS)(UINTN)VariableArrayAddress, MemoryResource, MemoryBase, MemorySize, ScratchBuffer, ScratchBufferSize);
  } else {
    //
    // Capsule is processed in IA32 mode.
    //
    Status = CapsuleDataCoalesce ((EFI_PHYSICAL_ADDRESS *)(UINTN)VariableArrayAddress, MemoryResource, MemoryBase, MemorySize, ScratchBuffer, ScratchBufferSize);
  }
#else
  //
  ...
  if (!EFI_ERROR(Status) && CompareGuid (ScratchBuffer, &gEfiMemoryRangeCapsuleGuid)) {
    MemoryRangeCapsule = ScratchBuffer;
    if (MemoryRangeCapsule->MemoryForFirmwareUse.Address != 0 && MemoryRangeCapsule->MemoryForFirmwareUse.Length != 0) {
      HobSize = sizeof(EFI_MEMORY_RANGE_CAPSULE);
    } else {
      HobSize = MemoryRangeCapsule->Header.CapsuleImageSize;
    }
    GuidHob = BuildGuidHob (&gEfiMemoryRangeCapsuleGuid, HobSize);
    CopyMem (GuidHob, MemoryRangeCapsule, HobSize);
  }
...
/**
  Build memory allocation hob based upon a MemoryRangeCapsule.
  As such the PEI or DXE core will not use these regions.
  These regions will be reported to be allocated by UEFI memory map to OS.
  The capsule will also be handled by DXE capsule module.

  @param[in]  MemoryRangeCapsule  A capsule image describe a list of scatter gather range which is reserved by OS.

  @retval EFI_SUCCESS The range is reserved successfully.
  @retval Error       Fail to reserve the range.
**/
EFI_STATUS
ReserveMemoryRange (
  ...
  )
{
  ...
  for (Index = 0; Index < MemoryRangeCapsule->NumberOfMemoryRanges; Index++) {
    DEBUG ((DEBUG_INFO, "BuildMemoryAllocationHob(0x%lx-0x%lx)\n", MemoryRangeCapsule->MemoryRanges[Index].Address, MemoryRangeCapsule->MemoryRanges[Index].Length));
    BuildMemoryAllocationHob (
      MemoryRangeCapsule->MemoryRanges[Index].Address,
      MemoryRangeCapsule->MemoryRanges[Index].Length,
      MemoryRangeCapsule->OsRequestedMemoryType
      );
  }
  ...
  )
{
  ...
  Hob.Raw = GetFirstGuidHob (&gEfiMemoryRangeCapsuleGuid);
  if (Hob.Raw != NULL) {
    MemoryRangeCapsule = GET_GUID_HOB_DATA (Hob);
    ASSERT (GET_GUID_HOB_DATA_SIZE (Hob) >= sizeof(EFI_MEMORY_RANGE_CAPSULE));

    if (MemoryRangeCapsule->MemoryForFirmwareUse.Address != 0 && MemoryRangeCapsule->MemoryForFirmwareUse.Length != 0) {
      MemoryRangeCapsule = (VOID *)(UINTN)MemoryRangeCapsule->MemoryForFirmwareUse.Address;
    } else {
      ASSERT (GET_GUID_HOB_DATA_SIZE (Hob) == MemoryRangeCapsule->Header.CapsuleImageSize);
    }

    Size = (UINTN)MemoryRangeCapsule->Header.CapsuleImageSize;
    ...
    ReserveMemoryRange (MemoryRangeCapsule);
    CopyMem ((VOID *)(UINTN)NewBuffer, MemoryRangeCapsule, Size);
    DEBUG ((DEBUG_INFO, "BuildCvHob(0x%lx-0x%x)\n", NewBuffer, Size));
  ...
  //
  DEBUG_CODE (
    CapsuleTestPattern ((VOID *) (UINTN) NewBuffer);
  );
