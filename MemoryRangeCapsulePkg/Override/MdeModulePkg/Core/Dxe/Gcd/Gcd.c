  ...
  )
{
  ...
  for (Hob.Raw = *HobStart; !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
    ...
    if (GET_HOB_TYPE (Hob) == EFI_HOB_TYPE_RESOURCE_DESCRIPTOR) {
      ...
      DEBUG ((EFI_D_INFO, "RESOURCE_DESCRIPTOR: (0x%x) 0x%lx-0x%lx\n", ResourceHob->ResourceType, ResourceHob->PhysicalStart, ResourceHob->ResourceLength));
      ...
      if (GcdMemoryType != EfiGcdMemoryTypeNonExistent) {
        ...
        DEBUG ((EFI_D_INFO, "CoreInternalAddMemorySpace: (0x%x) 0x%lx-0x%lx\n", GcdMemoryType, ResourceHob->PhysicalStart, ResourceHob->ResourceLength));
  ...
  for (Hob.Raw = *HobStart; !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
    if (GET_HOB_TYPE (Hob) == EFI_HOB_TYPE_MEMORY_ALLOCATION) {
      ...
      if (!EFI_ERROR (Status) &&
        ...
        //
        // There might be a case that PEI phase reserved memory by using memory allocation hob.
        // But it is marked as NOT_TESTED, so that the GCD type is Reserved.
        // We should convert it to EfiGcdMemoryTypeSystemMemory, so that it can be added into MemMap.
        //
        Status = CoreRemoveMemorySpace (BaseAddress, MemoryHob->AllocDescriptor.MemoryLength);
        DEBUG ((EFI_D_INFO, "CoreRemoveMemorySpace: (%r) 0x%lx-0x%lx\n", Status, BaseAddress, MemoryHob->AllocDescriptor.MemoryLength));
        Capabilities = Descriptor.Capabilities & (~(EFI_MEMORY_PRESENT|EFI_MEMORY_INITIALIZED|EFI_MEMORY_TESTED));
        Status = CoreInternalAddMemorySpace (EfiGcdMemoryTypeSystemMemory, BaseAddress, MemoryHob->AllocDescriptor.MemoryLength, Capabilities);
        DEBUG ((EFI_D_INFO, "CoreInternalAddMemorySpace: (%r) 0x%lx-0x%lx(0x%lx)\n", Status, BaseAddress, MemoryHob->AllocDescriptor.MemoryLength, Capabilities));
        Status = CoreGetMemorySpaceDescriptor (BaseAddress, &Descriptor);
      }
      if (!EFI_ERROR (Status)) {
        DEBUG ((EFI_D_INFO, "CoreAllocateMemorySpace: (0x%x) 0x%lx-0x%lx\n", Descriptor.GcdMemoryType, BaseAddress, MemoryHob->AllocDescriptor.MemoryLength));
        Status = CoreAllocateMemorySpace (
                   EfiGcdAllocateAddress,
                   Descriptor.GcdMemoryType,
                   0,
        ...
        if (!EFI_ERROR (Status) &&
            ((Descriptor.GcdMemoryType == EfiGcdMemoryTypeSystemMemory) ||
             (Descriptor.GcdMemoryType == EfiGcdMemoryTypeMoreReliable))) {
          DEBUG ((EFI_D_INFO, "CoreAddMemoryDescriptor: (0x%x) 0x%lx-0x%lx(0x%lx)\n", MemoryHob->AllocDescriptor.MemoryType, MemoryHob->AllocDescriptor.MemoryBaseAddress, MemoryHob->AllocDescriptor.MemoryLength, Descriptor.Capabilities));
          CoreAddMemoryDescriptor (
